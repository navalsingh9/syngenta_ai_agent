...
# Map human-friendly aliases to actual column names
column_aliases = {
    "amount": "sales",
    "sales_amount": "sales",
    "region": "order_region",
...
def ask_data_question(question: str):
    question = expand_quarter_dates(question)
    sql_query = sql_chain.run({"question": question}).strip().strip("```sql").strip("```").strip()
