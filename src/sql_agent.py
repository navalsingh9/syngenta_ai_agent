...
def normalize_text_match_columns(query: str, columns: list[str]) -> str:
    for col in columns:
        pattern = rf"{col}\s*=\s*'([^']+)'"
        query = re.sub(
            pattern,
            lambda m: f"LOWER({col}) LIKE '%{m.group(1).lower()}%'",
            query,
            flags=re.IGNORECASE
        )
    return query
...
def warn_if_ungrouped_columns(sql: str) -> str:
    select_cols = re.findall(r"SELECT\s+(.*?)\s+FROM", sql, re.IGNORECASE | re.DOTALL)
    group_by_cols = re.findall(r"GROUP\s+BY\s+(.*?)\s+(ORDER|LIMIT|$)", sql, re.IGNORECASE | re.DOTALL)
    if not select_cols or not group_by_cols:
        return sql  # nothing to do

    select_parts = [s.strip() for s in re.split(r",|\n", select_cols[0]) if s.strip() and "(" not in s]
    group_by_parts = [s.strip() for s in re.split(r",|\n", group_by_cols[0][0]) if s.strip()]

    ungrouped = [col for col in select_parts if col not in group_by_parts and not col.lower().startswith("sum")]
    if ungrouped:
...
def ask_data_question(question: str, llm):
    ...
    if not is_claude:
        sql_query = replace_column_aliases(sql_query, column_aliases)
        sql_query = patch_malformed_date_usage(sql_query)
        sql_query = normalize_text_match_columns(sql_query, TEXT_COLUMNS_TO_COLLATE)

        sql_query = warn_if_ungrouped_columns(sql_query)
